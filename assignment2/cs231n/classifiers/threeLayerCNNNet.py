...
class ThreeLayerCNNNet(object):


    def __init__(self, input_dim=(3, 32, 32), num_filters=[3,16,20,20],
                   convW=[5,5,5],seed=None,dropout=0,
                 num_classes=10, weight_scale=1e-2, reg=0.0,
                 dtype=np.float32, conv_relu_pool_Layer=3, h5_file=None):
        """
        ...
        self.conv_relu_pool_Layer = conv_relu_pool_Layer
    ...
    def save_weights(self,h5_file,verbose=False):
        with h5py.File(h5_file, 'w') as hf:
            ...
            for i in range(self.conv_relu_pool_Layer):
                hf.create_dataset('W%d' % i,data=self.params['W%d' % i])
                hf.create_dataset('b%d' % i, data=self.params['b%d' % i])
                H /= 2
                W /= 2
            hf.create_dataset('W%d' % self.conv_relu_pool_Layer, data=self.params['W%d' % self.conv_relu_pool_Layer])
            hf.create_dataset('b%d' % self.conv_relu_pool_Layer, data=self.params['b%d' % self.conv_relu_pool_Layer])

...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in xrange(self.num_layers):
      ...
      if self.use_batchnorm and self.num_layers - 1 != i:
  ...
  def loss(self, X, y=None):
    ...
    grads['W' + repr(last_layer)] += self.reg * self.params['W'+ str(last_layer)]
    regterms = [np.sum(self.params['W%i' % i] * self.params['W%i' % i]) for i in arange(self.num_layers)]
    ...
    for i in range(last_layer - 1, -1, -1):
      ...
      affine_cache = self.cache[a_name]

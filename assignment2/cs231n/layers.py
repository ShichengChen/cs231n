...
def conv_forward_naive(x, w, b, conv_param):
  ...
  kernel_size = C * HH * WW
  convolution = np.reshape(w, (F, kernel_size)).T
  ...
  for i in range(H_out):
    ...
    for j in range(W_out):
      ...
      view = np.reshape(X[:, :, top:bottom, left:right], (N, kernel_size))

      #TODO very elegant
      out[:, :, i, j] = view.dot(convolution) + b
      #X[N * ks] dot W[ks * F]
...
def conv_backward_naive(dout, cache):
  ...
  for i in range(H_out):
    ...
    for j in range(W_out):
      ...
      dout_ij = dout[:, :, i, j] # N * F * h * w
      x_sub = x_padded[:, :, top:bottom, left:right]
      x_sub_reshaped = x_sub.reshape(N, C * HH * WW)
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(H_out):
    ...
    for j in range(W_out):
      ...
      pos = np.argmax(x[:, :, top:bottom, left:right].reshape((N * C, pool_height * pool_width)), axis=1)
      cur = dx[:, :, top:bottom, left:right].reshape((N * C, pool_height * pool_width))
      #todo maybe we cannot reshape on the dx[:, :, top:bottom, left:right]
      cur[range(N * C), pos] += dout_ij
      dx[:, :, top:bottom, left:right] = cur.reshape(N, C, pool_height, pool_width)

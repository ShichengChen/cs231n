...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  for t in xrange(T):
    #TODO look out (_prev = h[t - 1])It's wrong
    if t == 0:
      h_prev = h0
    else:
      h_prev = h[t - 1]
...
def rnn_backward(dh, cache):
  ...
  for iter in reversed(xrange(T)):
    # TODO important
    dh_current = dh[iter] + dh_prev
    dx_t, dh_prev, dWx_t, dWh_t, db_t = rnn_step_backward(
      dh_current, cache[iter])
    dx[iter] += dx_t
    dh0 = dh_prev
    dWx += dWx_t
    dWh += dWh_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  cache = i, f, o, g, a, ai, af, ao, ag, Wx, Wh, b, prev_h, prev_c, x, next_c, next_h,tanh_next_c
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  #############################################################################
  i, f, o, g, a, ai, af, ao, ag, Wx, Wh, b, prev_h, prev_c, x, next_c, next_h,tanh_next_c = cache
  #next_c = f * prev_c + i * g
  #tanh_next_c = np.tanh(next_c)
  #next_h = o * tanh_next_c
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  for t in range(T):
    if t > 0:
      prev_h = h[t - 1]
      prev_c = c[t - 1]
    h[t], c[t], tcache = lstm_step_forward(x[t], prev_h, prev_c, Wx, Wh, b)
    cache.append(tcache)
  h = h.transpose(1, 0, 2)
  ##############################################################################
  #                               END OF YOUR CODE                             #
...
def lstm_backward(dh, cache):
  ...
  #############################################################################
  N, T, H = dh.shape
  i, f, o, g, a, ai, af, ao, ag, Wx, Wh, b, prev_h, prev_c, x, next_c, next_h, tanh_next_c = cache[0]
  D = x.shape[-1]

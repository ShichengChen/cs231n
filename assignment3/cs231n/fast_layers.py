...
def prepare_conv_cache(x, w, b, conv_param):
  N, C, H, W = x.shape
  F, _, HH, WW = w.shape
  stride, pad = conv_param['stride'], conv_param['pad']
...
def max_upPooling_backward_reshape(dout,cache):
  method, real_cache = cache
  x, x_reshaped, out = real_cache
  N, C, H, W = x.shape
  ...
  #print mask
  #print mask
  dout = dout.reshape(N, C, H / 2, 2, W / 2, 2)
  dout /= np.sum(mask, axis=(3, 5), keepdims=True)
  dx_reshaped[mask] = dout[mask]
  dx_reshaped = dx_reshaped.sum(axis=(3, 5))
  return dx_reshaped
